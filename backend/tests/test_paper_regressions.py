...
def short_detail(price: float = 100.0) -> dict:
    values = detail(price)
    values["spot"] = {"buy_pressure_5m": 1.0, "sell_pressure_5m": 3.0, "buy_volume_5m": 100.0, "sell_volume_5m": 800.0, "cvd_5m": -700.0}
    values["perp"] = {"buy_pressure_5m": 1.0, "sell_pressure_5m": 3.0, "buy_volume_5m": 100.0, "sell_volume_5m": 800.0, "cvd_5m": -700.0}
    return values
...
class PaperTradingRegressionTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_recovery_restores_recent_close_cooldown(self) -> None:
        repository = MemoryPaperRepository()
        now = datetime(2025, 1, 1, tzinfo=UTC)
        repository.rows[1] = {"id": 1, "symbol": "BTCUSDT", "status": "CLOSED", "closed_at": now - timedelta(minutes=2)}
    ...
    async def test_marks_favorable_and_adverse_excursion_before_exit(self) -> None:
        repository = MemoryPaperRepository()
        engine = PaperTradingEngine(repository, PaperTradingSettings(signal_persistence_seconds=0, take_profit_pct=1, stop_loss_pct=1))
        now = datetime(2025, 1, 1, tzinfo=UTC)
        await engine.process_update("BTCUSDT", detail(), now)
